<?php
    ...
    $row = $result->fetch_assoc();
     ...
     if (isset($_GET["search"])){  
        ...
        $search = $_GET['search'];
        $check_record = "SELECT * FROM vaccine WHERE lastname LIKE '$search' || firstname LIKE '$search' || middlename LIKE '$search' || userid LIKE '$search' || typeofvaccine LIKE '$search' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '%$search%' || place LIKE '$search' || status1 LIKE '$search' order by id desc";
        $sqlvalidate = mysqli_query($conn, $check_record);

        if (mysqli_num_rows($sqlvalidate) == 0){
        // echo "<script>alert('No existing data');</script>";
        // echo "<script>window.location.href = './List_of_schedule.php'</script>";
        $_SESSION['no_existing_data'] = "no_existing_data";
        }else{
        $search = $_GET['search'];

        $sql = "SELECT * FROM vaccine WHERE lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || typeofvaccine LIKE '%$search%' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '%$search%' || place LIKE '%$search%' || status1 LIKE '%$search%' order by id desc";
        $result = $conn->query($sql) or die ($conn->error);
        $row = $result->fetch_assoc();
...
<body>
    <div class="navigation" >
        ...
        <div class="navigation-button">
            ...
            <a href="List_of_schedule.php" style="background-color: #3083b3;">
                <div class="btn">
    ...
    <div class="container" id="container">
        ...
        <form method="GET" action="" class="personal-info-btn" id="personal-info-btn-1" autocomplete="off">
            ...
            <?php
                ...
                $search = $_GET['search'];
                $schedule_data = "SELECT * FROM vaccine where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || typeofvaccine LIKE '$search' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '%$search%' || place LIKE '%$search%' || status1 LIKE '$search'";
                $schedule_data_run = mysqli_query($conn, $schedule_data);
...
<script>
	...
	refresh.onclick =  function(){
        window.location.href = './List_of_schedule.php';
    }
