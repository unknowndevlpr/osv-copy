<?php
    ...
    $row = $result->fetch_assoc();
     ...
     if (isset($_GET["search"])){  
        ...
        $search = $_GET['search'];
        $check_record = "SELECT * FROM cancel_request where lastname LIKE '$search' || firstname LIKE '$search' || middlename LIKE '$search' || userid LIKE '$search' || typeofvaccine LIKE '$search' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '$search' || place LIKE '%$search%' || status1 LIKE '$search' || daterequest LIKE '$search' || cancelstatus LIKE '$search' || datecancel LIKE '$search'order by id desc";
        ...
        $sql = "SELECT * FROM cancel_request where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || typeofvaccine LIKE '$search' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '%$search%' || place LIKE '%$search%' || status1 LIKE '%$search%' || daterequest LIKE '$search' || cancelstatus LIKE '%$search%' || datecancel LIKE '%$search%'order by id desc";
        $result = $conn->query($sql) or die ($conn->error);
        $row = $result->fetch_assoc();
    ...
    <div class="container" id="container">
        ...
        <form method="GET" action="" class="personal-info-btn" id="personal-info-btn-1" autocomplete="off">
            ...
            <?php
                ...
                $blank = "";
                 if($search == $blank){
                $schedule_data = "SELECT * FROM cancel_request";
                $schedule_data_run = mysqli_query($conn, $schedule_data);
                ...
                $search = $_GET['search'];
                $schedule_data = "SELECT * FROM cancel_request where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || typeofvaccine LIKE '$search' || dosage LIKE '%$search%' || dateofvaccine LIKE '%$search%' || timeofvaccine LIKE '%$search%' || place LIKE '%$search%' || status1 LIKE '$search' || daterequest LIKE '$search' || cancelstatus LIKE '%$search%' || datecancel LIKE '%$search%'";
                $schedule_data_run = mysqli_query($conn, $schedule_data);
        ...
        <div class="table">
            ...
            <div id="a">
                <div class="table-column">
                    ...
                    <div class="table-column-row" id="table-header-row-resize">
                            <a href="Admin_cancel_request_view.php?id=<?php echo $row['unique_id'];?>" id="apply"><label>View</label></a> 
                        </div>
                </div>
            </div>
