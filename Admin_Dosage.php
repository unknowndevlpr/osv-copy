<?php
    ...
    $row = $result->fetch_assoc();
     ...
     if (isset($_GET["search"])){  
        ...
        $search = $_GET['search'];
        $check_record = "SELECT * FROM list where lastname LIKE '$search' || firstname LIKE '$search' || middlename LIKE '$search' || userid LIKE '$search' || firstdose LIKE '$search' || firstdosevaccine LIKE '$search' || seconddose LIKE '$search' || seconddosevaccine LIKE '$search' || booster LIKE '$search' || boostervaccine LIKE '$search' || secondbooster LIKE '$search' || secondboostervaccine LIKE '$search' order by id desc";
        ...
        $sql = "SELECT * FROM list where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || firstdose LIKE '%$search%' || firstdosevaccine LIKE '%$search%' || seconddose LIKE '%$search%' || seconddosevaccine LIKE '%$search%' || booster LIKE '%$search%' || boostervaccine LIKE '%$search%' || secondbooster LIKE '%$search%' || secondboostervaccine LIKE '%$search%' order by id desc";
        $result = $conn->query($sql) or die ($conn->error);
        $row = $result->fetch_assoc();
...
<body>
    ...
    <div class="container" id="container">
        ...
        <form method="GET" action="" class="personal-info-btn" id="personal-info-btn-1" autocomplete="off">
            ...
            <?php
                ...
                $search = $_GET['search'];
                $schedule_data = "SELECT * FROM list where lastname LIKE '%$search%' || firstname LIKE '%$search%' || middlename LIKE '%$search%' || userid LIKE '%$search%' || firstdose LIKE '%$search%' || firstdosevaccine LIKE '%$search%' || seconddose LIKE '%$search%' || seconddosevaccine LIKE '%$search%' || booster LIKE '%$search%' || boostervaccine LIKE '%$search%' || secondbooster LIKE '%$search%' || secondboostervaccine LIKE '%$search%'";
                $schedule_data_run = mysqli_query($conn, $schedule_data);
        ...
        <div class="table">
            ...
            <div id="a">
                <div class="table-column">
                    ...
                    <div class="table-column-row" id="table-header-row-resize">
                        <a href="Admin_Dosage_view.php?id=<?php echo $row['unique_id'];?>" id="apply"><label>View</label></a> 
                    </div>
                </div>
            </div>
    ...
    ?>
      <?php
        if(isset($_SESSION['save_dosage_successfully']) == "save_dosage_successfully"){
        ?>
        <script>
            Swal.fire({
                icon: 'success',
        ...
        <?php
            unset($_SESSION['save_dosage_successfully']);
        }
    ?>
